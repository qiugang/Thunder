...
public class Thunder {
     ...
     * us progress. 主线程接口， 用于接收处理来自后台线程的消息，可用于更新 UI 。
     */
     ...
     * equals MESSAGE_*. 当你在后台线程中发送消息的时候， 你的 message.what 不应该和 MESSAGE_* 相等。
     */
     ...
     * lifecycle, so we get state real-time :) 那么怎么样才能实时获得当前上下文环境的状态呢？ 很简单，只需要通过
     * Activity/FragmentActivity(v4)/Fragment/Fragment(v4) 的 FragmentManager
     * 加入一个 hook fragment ， 这个 hook fragment 的生命周期会跟随它所依附的上下文环境，
    ...
    public static class HookSupportFragment extends
     ...
     * MESSAGE); 使用这个方法，在后台线程中向主线程发送消息： SugarTask.post(YOUR MESSAGE);
     */
    @WorkerThread
     ...
     * decouple WorkerThread and MainThread, avoid OOM. 当 message.what =
     * MESSAGE_STOP ，我们立即清除所有在主线程中的回调， 这样就做到了后台线程和主线程的解耦，有效避免 OOM 。
     */
            ...
            try {
                ...
                call.enqueue(new Callback() {
                    ...
                    public void onResponse(Call call, Response response) throws IOException {
                        ...
                        message.obj = new Holder(id,
                                new ResponseData.Builder().code(response.code())
